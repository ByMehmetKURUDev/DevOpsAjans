...
The backend is a self-hosted FastAPI service on Render, backed by PostgreSQL, Google OIDC for auth and S3-compatible object storage.
...
- All API routes MUST use prefix `/api/v1/` to be correctly proxied by the Pages Function.
...
- IMPORTANT: DO NOT CREATE ANY USER TABLES. Users come from the OIDC provider; the app stores only the subject id.
