...
class Analytics_snapshotsService:
    ...
    async def get_list(
        ...
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Analytics_snapshots, field):
                        column = getattr(Analytics_snapshots, field)
            ...
            stmt = select(Analytics_snapshots, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Analytics_snapshots, field_name):
                        stmt = stmt.order_by(getattr(Analytics_snapshots, field_name).desc())
                else:
                    if hasattr(Analytics_snapshots, sort):
                        stmt = stmt.order_by(getattr(Analytics_snapshots, sort))
            else:
                stmt = stmt.order_by(Analytics_snapshots.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Analytics_snapshots)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Analytics_snapshots]:
        ...
        try:
            ...
            column = getattr(Analytics_snapshots, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Analytics_snapshots).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
