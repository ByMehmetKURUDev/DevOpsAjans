...
class InvoicesService:
    ...
    async def get_list(
        ...
        try:
            ...
            stmt = select(Invoices, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Invoices, field_name):
                        stmt = stmt.order_by(getattr(Invoices, field_name).desc())
                else:
                    if hasattr(Invoices, sort):
                        stmt = stmt.order_by(getattr(Invoices, sort))
            else:
                stmt = stmt.order_by(Invoices.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Invoices)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Invoices]:
        ...
        try:
            ...
            column = getattr(Invoices, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Invoices).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
