...
# ------------------ Service Layer ------------------
class Support_ticketsService:
    """Service layer for Support_tickets operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Support_tickets]:
        """Create a new support_tickets"""
        try:
            obj = Support_tickets(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created support_tickets with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating support_tickets: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Support_tickets]:
        """Get support_tickets by ID"""
        try:
            query = select(Support_tickets).where(Support_tickets.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching support_tickets {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of support_ticketss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Support_tickets, field):
                        column = getattr(Support_tickets, field)
            ...
            stmt = select(Support_tickets, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Support_tickets, field_name):
                        stmt = stmt.order_by(getattr(Support_tickets, field_name).desc())
                else:
                    if hasattr(Support_tickets, sort):
                        stmt = stmt.order_by(getattr(Support_tickets, sort))
            else:
                stmt = stmt.order_by(Support_tickets.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Support_tickets)
                for condition in conditions:
            ...
            logger.error(f"Error fetching support_tickets list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Support_tickets]:
        """Update support_tickets"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Support_tickets {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated support_tickets {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating support_tickets {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Support_tickets {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted support_tickets {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting support_tickets {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Support_tickets]:
        """Get support_tickets by any field"""
        try:
            if not hasattr(Support_tickets, field_name):
                raise ValueError(f"Field {field_name} does not exist on Support_tickets")
            column = getattr(Support_tickets, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Support_tickets).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching support_tickets by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Support_tickets]:
        """Get list of support_ticketss filtered by field"""
        try:
            if not hasattr(Support_tickets, field_name):
                raise ValueError(f"Field {field_name} does not exist on Support_tickets")
            column = getattr(Support_tickets, field_name)
            ...
            logger.error(f"Error fetching support_ticketss by {field_name}: {str(e)}")
            raise